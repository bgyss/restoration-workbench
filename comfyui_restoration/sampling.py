...
def plan_samples(duration_seconds: float, *, chapters: Iterable[float] = (), defect_times: Iterable[float] = (), manual_times: Iterable[float] = (), sample_duration: float = 60.0) -> list[dict]:
    ...
    candidates: list[tuple[float, str]] = [(duration_seconds * fraction, reason) for fraction, reason in ((0.01, "early runtime coverage"), (0.5, "middle runtime coverage"), (0.9, "late runtime coverage"))]
    candidates.extend((float(value), "chapter boundary") for value in chapters)
    candidates.extend((float(value), "detected defect") for value in defect_times)
    candidates.extend((float(value), "manual addition") for value in manual_times)
    result: list[dict] = []
    seen: set[int] = set()
    half = sample_duration / 2
    for timestamp, reason in sorted(candidates):
